...
// --- Visitor Categories Endpoints ---
app.get('/api/visitors/categories', async (req, res) => {
  try {
    const pool = getPool();
    const [rows] = await pool.query('SELECT * FROM visitor_categories ORDER BY category_name ASC');
    res.json(rows);
  } catch (error) {
...
app.post('/api/visitors/categories', async (req, res) => {
  const { categoryName, defaultAmount } = req.body;
  if (!categoryName) {
    return res.status(400).json({ error: 'Category name is required' });
  }
  try {
    const pool = getPool();
    const query = 'INSERT INTO visitor_categories (category_name, default_amount) VALUES (?, ?)';
    await pool.query(query, [categoryName, parseFloat(defaultAmount || 0)]);
...
app.delete('/api/visitors/categories/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const pool = getPool();
    const query = 'DELETE FROM visitor_categories WHERE id = ?';
...
app.get('/api/visitors/passes', async (req, res) => {
  try {
    const pool = getPool();
    const [rows] = await pool.query('SELECT * FROM visitor_passes ORDER BY id DESC');
    res.json(rows);
  } catch (error) {
...
app.get('/api/visitors/passes/next-code', async (req, res) => {
  try {
    const pool = getPool();
    const [rows] = await pool.query('SELECT pass_code FROM visitor_passes ORDER BY id DESC LIMIT 1');
    let nextNum = 1001;
    if (rows.length > 0) {
      const lastCode = rows[0].pass_code;
      const match = lastCode.match(/\d+/);
      if (match) {
        nextNum = parseInt(match[0], 10) + 1;
      }
    }
    const nextCode = `FSV-${nextNum}`;
    res.json({ nextCode });
  } catch (error) {
    console.error('Error generating next pass code:', error);
    res.status(500).json({ error: 'Failed to generate next pass code' });
  }
});

app.post('/api/visitors/passes', async (req, res) => {
  const { visitorName, mobileNumber, categoryName, adultsCount, childrenCount, passAmount, cafeCoupon, status, issuedDate } = req.body;
  if (!visitorName || !mobileNumber || !categoryName || !issuedDate) {
    return res.status(400).json({ error: 'Missing required visitor pass fields' });
  }
  try {
    const pool = getPool();
    ...
    const passCode = `FSV-${nextNum}`;

    const query = `
      INSERT INTO visitor_passes 
      (pass_code, visitor_name, mobile_number, category_name, adults_count, children_count, pass_amount, cafe_coupon, status, issued_date)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    await pool.query(query, [
      passCode,
      ...
      mobileNumber,
      categoryName,
      parseInt(adultsCount || 1, 10),
      parseInt(childrenCount || 0, 10),
      parseFloat(passAmount || 0),
      cafeCoupon || 'None',
      status || 'Checked In',
      issuedDate
    ]);
    res.status(201).json({ success: true, message: 'Visitor pass issued successfully!' });
  } catch (error) {
...
app.put('/api/visitors/passes/:id', async (req, res) => {
  const { id } = req.params;
  const { visitorName, mobileNumber, categoryName, adultsCount, childrenCount, passAmount, cafeCoupon, status, issuedDate } = req.body;
  try {
    const pool = getPool();
    if (visitorName === undefined) {
      const query = 'UPDATE visitor_passes SET status = ? WHERE id = ?';
    ...
    const query = `
      ...
      SET visitor_name = ?, mobile_number = ?, category_name = ?, adults_count = ?, children_count = ?, pass_amount = ?, cafe_coupon = ?, status = ?, issued_date = ?
      WHERE id = ?
    `;
    const [result] = await pool.query(query, [
      visitorName,
      mobileNumber,
      categoryName,
      parseInt(adultsCount, 10),
      parseInt(childrenCount, 10),
      parseFloat(passAmount),
      cafeCoupon,
      status,
      issuedDate,
      parseInt(id, 10)
...
app.delete('/api/visitors/passes/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const pool = getPool();
    const query = 'DELETE FROM visitor_passes WHERE id = ?';
    const [result] = await pool.query(query, [parseInt(id, 10)]);
